              ...
              if(substr.length() > 0){
                if(lines_read>0){
                  ...
                  if(first_int){
                      ...
                      second_int=true;
					  std::cout << "Mrow: " << csc_matrix.num_rows << ", Mcol: " << csc_matrix.num_cols<< std::endl;
                  } else {
...
std::vector<float> SubRoutine002Filter_opt(aligned_vector<PACKED_VAL_T> &rv, uint32_t sz){
    std::vector<float> new_vector;
    //=rv - ((dot(rv, on) / dot(on, on)) * on)
    float dot_product;
    float mean_dot_product;
    ...
    new_vector=std::vector<float>(sz);
    dot_product=0;
    uint32_t terms_covered;
    terms_covered=0;
    for(uint32_t i=0;i<rv.size();i++){
        for(uint32_t k=0;k<PACK_SIZE;k++){
            terms_covered++;
            dot_product+=float(rv[i].data[k]);
            if(terms_covered==sz)
                break;
        }
    }
    ...
    for(uint32_t i=0;i<rv.size();i++){
        for(uint32_t k=0;k<PACK_SIZE;k++){        
            new_vector[terms_covered++]=float(rv[i].data[k])-mean_dot_product;
            if(terms_covered==sz)
                break;
        }
    }
