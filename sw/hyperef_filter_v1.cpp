...
bool hyperef_filter_kernel_accel(cl_runtime &runtime,
		       uint32_t mx,
                       ...
                       spmv::io::CSRMatrix<float> &D, std::vector<float> &sm,
                       std::vector<float> &sm_ot, std::vector<float> &sm_norm,
  ...
  // std::generate(vector_f.begin(), vector_f.end(), [&](){return float(rand() %
  // 2);}); aligned_vector<PACKED_VAL_T> vector(mat.num_cols / PACK_SIZE);
  ...
  uint32_t sz = sm.size();
  sm.resize((sm_ping.size() * PACK_SIZE));
  ...
  for (uint32_t loop = 0; loop < loop_count; loop++) {
	...
	for (size_t i = 0; i < sm_ping.size(); i++) {
		for (size_t k = 0; k < PACK_SIZE; k++) {
			sm_ping[i].data[k] = VAL_T(sm[i * PACK_SIZE + k]);
			// sm_ping[i].data[k] = VAL_T(-1.24);
			sm_pong[i].data[k] = VAL_T(0);
			// cout << float(sm_ping[i].data[k]) << "," << sm[i * PACK_SIZE + k] << "," << i * PACK_SIZE + k   << endl;
		}
	}
    ...
    for (size_t row_part_id = 0; row_part_id < num_row_partitions;
	  ...
	  // std::cout << "INFO: submitting queue" << std::endl;
      runtime.command_queue.finish();
      // std::cout << "INFO : SpMV Kernel Finished: row partition " << row_part_id
                // << std::endl;
    }
	//https://xilinx.github.io/XRT/master/html/m2m.html
    runtime.command_queue.enqueueMigrateMemObjects({pong_buf}, CL_MIGRATE_MEM_OBJECT_HOST);
	runtime.command_queue.finish();
    // std::cout << "INFO : SpMV kernel complete!" << std::endl;
	// cout << "SM After" << endl;
    // transferH2H(sm_pong, sm_ping);
    unpack_vector_flt(sm_pong, sm);
	
	// return false;
	// cout << "SM after 1 SpMv operation " << sm_ping.size() << endl;
	for (size_t i = 0; i < sm_ping.size(); i++) {
		for (size_t k = 0; k < PACK_SIZE; k++) {
			sm_ping[i].data[k] = VAL_T(sm[i * PACK_SIZE + k]);
			// sm_pong[i].data[k] = VAL_T(0);
    ...
    OCL_CHECK(err, err = runtime.command_queue.enqueueMigrateMemObjects(
                       {ping_buf}, 0 /* 0 means from host*/));
    runtime.command_queue.finish();
	// cout << "Killed here...no" << sm_ping.size() << endl;
    // ping=AD*pong
    for (size_t c = 0; c < SK0_CLUSTER; c++) {
    ...
    // transferH2H(sm_pong, sm_ping);
    unpack_vector_flt(sm_pong, sm);
	// cout << "SM after 2 SpMv operation" << endl;
	for (size_t i = 0; i < sm_ping.size(); i++) {
		for (size_t k = 0; k < PACK_SIZE; k++) {
			sm_ping[i].data[k] = VAL_T(sm[i * PACK_SIZE + k]);
			// sm_pong[i].data[k] = VAL_T(0);
			// cout << "sm==>" << sm[i * PACK_SIZE + k] << "," << i * PACK_SIZE + k   << endl;
		}
	}
    OCL_CHECK(err, err = runtime.command_queue.enqueueMigrateMemObjects(
                       {ping_buf}, 0 /* 0 means from host*/));
    ...
    runtime.command_queue.finish();
    unpack_vector_flt(sm_pong, sm);
	// cout << "SM after 3 SpMv operation" << endl;
	for (size_t i = 0; i < sm_ping.size(); i++) {
		for (size_t k = 0; k < PACK_SIZE; k++) {
			sm_ping[i].data[k] = VAL_T(sm[i * PACK_SIZE + k]);
			// sm_pong[i].data[k] = VAL_T(0);
    ...
    sm_ot = SubRoutine002Filter(sm,sz);
    /*for (uint32_t j = 0; j < sz; j++) {
	  cout << "Writing sm_ot" << j << " : " << sm_ot[j] << std::endl;
    }*/

    // sm_norm = sm_ot ./ norm(sm_ot);
    sm_norm = SubRoutine003Filter(sm_ot,sz);
    /*for (uint32_t j = 0; j < sz; j++) {
	  cout << "Writing sm_norm____all" << j << " : " << sm_norm[j] << std::endl;
    }*/
    for (uint32_t j = 0; j < mx; j++) {
      sm_vec[j][loop] = sm_norm[j];
	  // cout << "Writing sm_vec["<<j<<"][loop]" << " : " << sm_vec[j][loop] << std::endl;
    }
    ///---// pack sm back to ping
    ///---for (size_t i = 0; i < sm_ping.size(); i++) {
    ///---  for (size_t k = 0; k < PACK_SIZE; k++) {
    ///---    sm_ping[i].data[k] = VAL_T(sm[i * PACK_SIZE + k]);
    ///---  }
...
std::vector<std::vector<float>>Filter_hw(cl_runtime &runtime, std::vector<float> rv, uint32_t k, spmv::io::CSCMatrix <float> &AD, uint32_t mx, uint32_t initial, uint32_t interval, uint32_t Ntot){
	...
	bool status =  hyperef_filter_kernel_accel(runtime,mx,
                       AD_dash,
                       D_dash, sm,
                       sm_ot, sm_norm,
                       sm_vec,
					   k);
	sm.resize(AD.num_rows);
	sm_ot.resize(AD.num_rows);
	sm_norm.resize(AD.num_rows);
	// std::cout << "I am here " << sm_vec.size() << std::endl;
	
    /*
	for(uint32_t loop=0;loop<k;loop++){
        //sm = D * sm
        sm = SubRoutine004Filter(D_dash,sm);

        //sm = AD * sm
        sm = SubRoutine004Filter(AD_dash,sm);
    ...
    for(uint32_t j=0;j<mx;j++){
      ...
      // for (uint32_t j = 0; j < mx; j++) {
		  // std::cout << V[j][Ntot-1] << j << "," << Ntot-1 << std::endl;
      // }

	// std::cout << "Welll ... I am here " << V.size() << std::endl;
    return V;
}
